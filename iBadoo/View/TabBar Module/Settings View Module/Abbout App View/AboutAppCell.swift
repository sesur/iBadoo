...
class AboutAppCell: BaseCell {
    
    var aboutAppCellObserver: SettingsModel? {
        didSet {
            guard let wrapper = aboutAppCellObserver else {return}
            titleLabel.attributedText = AttributedStringHelper.getStringNoneColor(text: wrapper.nameLabel.rawValue, fontType: .regular, size: 17)
