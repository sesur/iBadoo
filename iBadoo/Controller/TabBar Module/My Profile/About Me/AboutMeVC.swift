...
class AboutMeViewController: UIViewController {
    ...
    var myProfileItem: MyProfileModel? {
        didSet {
            navigationItem.title = myProfileItem?.nameLabel.rawValue
        }
    }
    
    //MARK:- Proprietes
    private lazy var relashionshipView: RelashionshipView = {
        let view = RelashionshipView()
        return view
    }()
    private let knowLanguageView: KnowLanguageView = {
        let view = KnowLanguageView()
        return view
    }()
    private let interestsView: InterestsView = {
    ...
    private func setupLayouts() {
        ...
        interestsView.anchor(top: contentView.topAnchor, leading: contentView.leadingAnchor, bottom: contentView.bottomAnchor, trailing: contentView.trailingAnchor, padding: .init(top: 460, left: 0, bottom: 8, right: 0), size: .init(width: 0, height: 500))
    }
}
